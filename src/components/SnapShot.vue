...
const cols=[{title:'快照名称',dataIndex:'name',key:'name',width:'28%'}
  ,{title:'快照时间',dataIndex:'ts',key:'ts',width:'25%'},
  {title:'快照状态',dataIndex:'status',key:'status',width:'22%'},
...
<template>
  <main>
    ...
    <a-row :gutter="[16,16]">
      <a-col span="24">
        <a-alert
          message="快照是一种数据保护技术，通过在某一特定时间点记录系统或存储卷的完整状态，实现对数据的即时冻结与保存，可用于快速恢复、版本回溯或测试环境创建，而不会影响业务的正常运行。"
      ...
      <a-col span="24">
        <div class="control">
          <div class="tools">
            <a-button type="primary" @click="onCreate">添加快照</a-button>
            <a-button type="primary" danger :disabled="!selectedRowKeys.length">删除</a-button>
            <a-button @click="onReload"><ReloadOutlined/></a-button>
            <a-typography-text type="secondary">总可创建快照数目：20000，已创建快照数：322</a-typography-text>
          </div>
          <div class="blank"></div>
